...
- Child(자식) selector : HTML그의 포함관계를 통해 부모 자식 태그로 구분하여 선택, 부모와 자식을 >으로 구분
- Sibling selector (인전한 형제 선택자) : HTML 태그의 포함관계들 중 같은 레벨에 있는 태그들을 형제로 구분, 시작태그 + 형제(인접한) 태그, 시작 태그 ~ 형제(모든 형제) 태그 (모든 형제 태그 의미함)
